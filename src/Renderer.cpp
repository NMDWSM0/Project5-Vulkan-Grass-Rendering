...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    VkDescriptorSetLayoutBinding inBladesLayoutBinding = {};
    inBladesLayoutBinding.binding = 0;
    inBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    inBladesLayoutBinding.descriptorCount = 1;
    inBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    inBladesLayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding outBladesLayoutBinding = {};
    outBladesLayoutBinding.binding = 1;
    outBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    outBladesLayoutBinding.descriptorCount = 1;
    outBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    outBladesLayoutBinding.pImmutableSamplers = nullptr;
    ...
    std::vector<VkDescriptorSetLayoutBinding> bindings = { inBladesLayoutBinding, outBladesLayoutBinding, numBladesLayoutBinding };
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        // Blades (compute)
        // NOTE: One Blades object already holds NUM_BLADES grass in it, so there will be one descriptor each Blades
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool;
    allocInfo.descriptorSetCount = scene->GetBlades().size();
    allocInfo.pSetLayouts = layouts;
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        VkDescriptorBufferInfo inBladesBufferInfo = {};
        inBladesBufferInfo.buffer = scene->GetBlades()[i]->GetBladesBuffer();
        inBladesBufferInfo.offset = 0;
        inBladesBufferInfo.range = NUM_BLADES * sizeof(Blade);

        VkDescriptorBufferInfo outBladesBufferInfo = {};
        outBladesBufferInfo.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
        outBladesBufferInfo.offset = 0;
        outBladesBufferInfo.range = NUM_BLADES * sizeof(Blade);
        ...
        descriptorWrites[3 * i + 0].pBufferInfo = &inBladesBufferInfo;
        ...
        descriptorWrites[3 * i + 1].pBufferInfo = &outBladesBufferInfo;
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        clearValues[0].color = { 0.5f, 0.635f, 0.743f, 1.0f };
